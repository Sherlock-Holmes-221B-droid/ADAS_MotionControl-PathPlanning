...
from Controllers.pure_pursuit_controller           import *
...
for _ in range(500):
    if pp.old_idx >= len(path.x) - 15:
        print(f"Finished tracking! Stopping safely at x = {vehicle_run.state.x:.2f}")
        break

    accel = pid.compute_control(target_speed=5.55, current_speed=vehicle_run.state.v, dt=0.1)
    desired_steer, tracking_metrics = pp.compute_steering(vehicle_run.state, path) 
    ...
    closest_points_history.append(tracking_metrics["closest_point"])
...
animation_data = {
    ...
    "title": "Pure Pursuit - Dynamic Lookahead"
}
if __name__ == "__main__":
    ...
    ani = FuncAnimation(
        ...
        lambda frame: plotter.animate(
            ...
            state_snapshots,
            history_snapshots,
            title_name="Pure Pursuit - Dynamic Lookahead"
        ),
